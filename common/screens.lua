...
local mFlags={ Flags = {FMENU_WRAPMODE=1,FMENU_AUTOHIGHLIGHT=1}, Title='Screens',
...
--      local etype= wintype=='E' and ('[ %s ]'):format(editors[i]) or '        '
        ...
        wind.text=('%2i. %s %s %s%s'):format(i, wintype, wind.Modified and '*' or ' ',
                                             wind.Current and '>' or ' ', title or 'none')
        wind.i=i
    ...
    elseif bkeys[item] then
        ...
        if key==bClose and (typ==F.WTYPE_VIEWER or typ==F.WTYPE_EDITOR) and not item.Modified then
            far.AdvControl('ACTL_SETCURRENTWINDOW',item.i)
            far.AdvControl('ACTL_COMMIT')
            if typ==F.WTYPE_EDITOR then editor.Quit(item.Id) else viewer.Quit(item.Id) end
            far.AdvControl('ACTL_COMMIT')
        ...
        elseif key==bGoto and (typ==F.WTYPE_VIEWER or typ==F.WTYPE_EDITOR) then
            ...
            for i=1, pinfo.ItemsNumber do
                ...
                if item.FileName==sname then
                    panel.RedrawPanel(nil, 1, { CurrentItem=i, TopPanelItem=
                        ( i>=topitem and i<(topitem+hheight) and topitem or i>hheight and i-hheight or 1 ) })
                    break
                end
            end
            if active.Type==F.WTYPE_PANELS then far.AdvControl('ACTL_COMMIT') end
        end
        return show_windows()
    end
end
