...
local function do_the_job(ftable)
    ...
    local blk_start_str= blocktype~=flags.BTYPE_NONE and EditorGetString(nil,editinfo.BlockStartLine,0) or nil
    if cfg.left and cfg.right then
        if blocktype==flags.BTYPE_COLUMN then
            commenter=ftable.block
        elseif blocktype==flags.BTYPE_NONE and not cfg.line then
            commenter=ftable.line_lr
        elseif not cfg.line or (blocktype==flags.BTYPE_STREAM and blk_start_str.SelStart~=0) then
    ...
    for i,str in farselection(true) do status,line,laststr,displ=commenter(i,str) end
    if status then
